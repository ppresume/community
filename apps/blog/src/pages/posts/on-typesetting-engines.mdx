...
- Simplified Chinese: [排版引擎纵谈：程序员的视角](/posts/zh-cn/on-typesetting-engines)
- Traditional Chinese TW: [排版引擎縱談：程式設計師的視角](/posts/zh-tw/on-typesetting-engines)
...
typesetting engines: HTML/CSS, [LaTeX](https://www.latex-project.org/),
[LaTeX.js](https://latex.js.org/), [Typst](https://typst.app/),
...
- **Latin script languages**: languages that use
  ...
  languages like [Indonesian](https://en.wikipedia.org/wiki/Indonesian_language)
...
- **[Justification](https://en.wikipedia.org/wiki/Typographic_alignment#Justified)**:
  ...
  letters, creating a uniform appearance across each line of text.
- **[Rag](https://www.myfonts.com/pages/fontscom-learning-fontology-level-2-text-typography-rags-widows-orphans)**:
  ...
  resulting in the opposite side appearing "ragged" or uneven.
...
for [SEO](https://en.wikipedia.org/wiki/Search_engine_optimization) and
...
dynamic PDF generation with [react](https://react.dev/). The choice of
...
There are 3 key metrics that are used to assess the quality of a line breaking
algorithm:

1. **Justification**: line breaking algorithms work in conjunction with
   [justification](https://en.wikipedia.org/wiki/Typographic_alignment#Justified)
   techniques to create evenly spaced lines of text.
2. **Hyphenation**: many advanced algorithms incorporate hyphenation to improve
   line breaks, especially for languages with long words.
...
than Latin script languages. Here is a classic
...
following [W3C](https://www.w3.org/) draft notes to get an intuitive sense of
the complexity of typesetting requirements for CJK:
...
languages is much more larger than Latin script languages. According to the
...
insanely huge**. In contrast, the [Latin
alphabet](https://en.wikipedia.org/wiki/Latin_alphabet) only has a few hundred
characters, much smaller than CJK. Hmmmm, 100k characters, even creating a font
that covers all of them is a huge amount of work, labor-intensive and very
expensive.

![CJK Characters](/static/images/on-typesetting-engines/The_old_man_is_72_years_old_final.webp)

Taking PPResume as an example, we met two issues
([1](https://github.com/ppresume/community/issues/33),
[2](https://github.com/ppresume/community/issues/63)) where the fonts
recommended by [CTeX](https://ctan.org/pkg/ctex?lang=en) are missing some
characters. Unlike Latin script languages, there are very few fonts that have
...
When mixing CJK with other Latin script languages, things become more
complicated.
...
Meanwhile, a Latin font may cover only one thousand glyphs, whereas a CJK font
must cover at least thousands of glyphs, as mentioned above.
...
In summary, the insanely huge size of CJK character sets, cultural nuances and
technical challenges contribute to the greater complexity of typesetting CJK
languages compared to Latin script languages.
...
- **Font selection**: HTML doesn't have built-in mechanisms for selecting
  specific fonts.
...
- CSS is used to style the HTML elements, controlling
  aspects like:
  - Font selection: specify fonts, font sizes, and font styles.
  - Text formatting: control line spacing, letter spacing, text alignment, and
    more.
  - Layout: create complex layouts using techniques like
...
([oh-my-cv](https://github.com/Renovamen/oh-my-cv) provides a good reference
...
[responsive](https://alistapart.com/article/responsive-web-design/), flow like
water, can adapt
[viewports](https://developer.mozilla.org/en-US/docs/Glossary/Viewport) of any
size.
...
to its limit. It uses `text-align: justify` and `hyphens: auto` to get an
...
- Pros
  ...
  - **Responsive**: HTMl & CSS is responsive and can adapt to viewport of any
    size
...
- Cons
  - **Limited control over typesetting**: compared to dedicated typesetting
    engines, HTML/CSS offers less control over fine typographic details.
  ...
  - **Poor line breaking**: as mentioned, HTML & CSS do not implement Knuth
    Plass line breaking algorithm
...
`hyphens: manual` for better line breaking.
...
LaTeX.js supports CJK because it is just a transpiler on top of HTML & CSS.
...
- horizontal
  [glue](https://www.overleaf.com/learn/latex/Articles/Boxes_and_Glue%3A_A_Brief%2C_but_Visual%2C_Introduction_Using_LuaTeX),
  like [`\hfill`](https://latexref.xyz/_005chfill.html) in a paragraph of text, is
  not possible
...
[web app](https://typst.app) run in a browsers via WebAssembly. However,
...
Last but not least, having fun with [polytype](https://polytype.dev/), a
[Rosetta Stone](<https://en.wikipedia.org/wiki/Rosetta_Stone_(software)>) for
typesetting engines.
...
This post [featured](https://news.ycombinator.com/item?id=42100660) by Hacker News.

To respond to some comments here.

#### Indo-European languages

[sundarurfriend](https://news.ycombinator.com/user?id=sundarurfriend) pointed
out that the usage of [Indo-European
languages](https://news.ycombinator.com/item?id=42103711) is inappropriate, and
he is right.

I am not a linguist and the two languages that I know well are Chinese and
English. So I have coined a new glossary term "Latin script languages" and use
that throughout the post instead.

#### Choice of typesetting engines

Some people asked me why I did not mention/evaluate xxx, yyy typesetting
engines. As I mentioned above, I did the evaluation based on PPReseume's
requirements. I chose the above 5 typesetting engines because each represents a
different type:

- [HTML & CSS](#html--css): traditional Web based typesetting/layout engine.
- [LaTeX](#latex): traditional CLI based typesetting engine. LaTeX is the most
  widely used, others including,
  [ConTeXt](https://wiki.contextgarden.net/Main_Page),
  [SILE](https://sile-typesetter.org/), [troff](https://troff.org/), etc.
- [LaTeX.js](#latexjs): an attempt to run LaTeX on the Web using JavaScript,
  interesting practice but very difficult to make it 100% compatible with LaTeX.
- [Typst](#typst): a modern and redesigned typesetting engine, with better
  syntax, user experience, incremental compilation, can be used on both CLI and
  web, ery promising project LaTeX alternative.
- [React-pdf](#react-pdf): react-based typesetting and PDF generation library.

#### text-autospace

Chrome has developed a new
[text-autospace](https://developer.chrome.com/blog/css-i18n-features#inter-script_spacing_text-autospace)
CSS property that can insert inter-script spacing by default. However, this
property appears to be available only in Chrome and is currently behind a feature
flag.
...
- add Chinese translations:
  - Simplified Chinese: [排版引擎纵谈：程序员的视角](https://blog.ppresume.com/posts/zh-cn/on-typesetting-engines)
  - Traditional Chinese TW: [排版引擎縱談：程式設計師的視角](https://blog.ppresume.com/posts/zh-tw/on-typesetting-engines)
