...
為了生成頂級、高質量的 PDF，排版引擎必須具備優秀的[換行演算法](https://en.wikipedia.org/wiki/Line_wrap_and_word_wrap)，而為了實現多語言的原生支援，排版引擎必須能夠處理具有龐大字符集的語言（如中文、日文和韓文，即 CJK）。在深入探討具體的排版引擎之前，讓我們先評估這兩個標準。
...
另，如前所述，印歐語言的拉丁字型可能覆蓋一千個字形，而 CJK 字型必須至少覆蓋數千個字形。
...
[LaTeX](https://www.latex-project.org/) 是一個建立在 TeX 之上的文件編制系統。它由 [Leslie Lamport](https://www.lamport.org/) 在 20 世紀 80 年代初建立，以簡化文件準備過程。LaTeX 在 TeX 的低階程式語言之上提供了一組更高階的宏，使其更易於使用和更直觀。
...
與許多其他系統使用的“首次擬合”方法不同，Knuth Plass 斷線演算法採用的是“總體擬合”換行演算法。這意味著：
...
總體而言，TeX 的換行演算法被認為是排版中最複雜和有效的方法之一，其核心原則持續影響現代排版系統，並在高質量數字排版的前沿保持領先地位。

### CJK

關於 CJK 排版，LaTeX 在一些新引擎和一些包的幫助下對 CJK 提供了相當好的支援：
...
總體而言，LaTeX 的 CJK 支援現在相當成熟，儘管在不同環境中設定可能需要一些時間。這裡有一頁來自 [The XeTeX Companion TEX meets OpenType and Unicode](https://xml.web.cern.ch/XML/lgc2/xetexmain.pdf) 的手冊文件，你可以一瞥 XeTeX 在 CJK 排版方面的能力。
...
儘管上述專案都沒有積極維護，但理論上可以在瀏覽器中純粹執行 LaTeX。這將大大減少從瀏覽器到伺服器的往返時間，我們可以獲得實時預覽。

### 結論

在總結之前，我想在這裡分享一些題外話。市場上基於 LaTeX 的簡歷生成器選擇非常少：
...
從商業角度來看，這是一個利基市場，並不太擁擠，所以可能值得我建立另一個基於 LaTeX 的簡歷生成器。

好了，是時候給 LaTeX 下個結論了。

- 優點
  - **精確和控制**：LaTeX 提供無與倫比的文件佈局和排版控制。
  - **黃金換行**：Knuth Plass 換行演算法是最優換行演算法的黃金標準，且其是由 TeX 作者發明的。
  - **廣泛的 CJK 支援**：有大量的宏包擴充套件了 LaTeX 對 CJK 的支援。
- 缺點
  - **學習曲線陡峭**：與所見即所得編輯器相比，LaTeX 對新使用者的入門門檻更高。
  - **沒有實時預覽**：預設情況下，LaTeX 需要在伺服器上進行編譯，因此沒有實時預覽。
  - **舊且神秘的開發體驗**：LaTeX 的編譯日誌有時難以閱讀，只能透過二分搜尋方法進行除錯。
...
> Typst 可以編譯為 WASM，但沒有可用的 JS 繫結，你需要自己編寫。它並不像 compile(string) 那樣簡單，因為你還需要提供字型，如果你想要多檔案設定，當然還需要檔案。
...
React-pdf 內部[實現了](https://react-pdf.org/advanced#hyphenation) Knuth Plass 換行演算法。預設情況下，它會對英語單詞進行斷詞符處理。

以下是來自 [react-pdf playground](https://react-pdf.org/repl) 示例文件的一頁，你可以注意到段落的佈局，文字整體看起來平衡且齊整，遠比普通 HTML 和 CSS 中的段落要好。
...
實際上，有一個開源的簡歷生成器 [open-resume](https://www.open-resume.com/resume-builder)，它使用這個庫實時生成和更新簡歷 PDF，你可以自己檢查輸出 PDF，並與 [LaTeX 生成的 PDF](https://ppresume.com/gallery) 進行比較。
